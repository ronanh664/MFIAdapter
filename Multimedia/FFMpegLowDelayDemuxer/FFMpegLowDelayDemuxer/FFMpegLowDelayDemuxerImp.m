...
#import "FFMpegLowDelayDemuxerImp.h"
...
@interface FFMpegLowDelayDemuxerImp()
{
...
@implementation FFMpegLowDelayDemuxerImp
...
static uint8_t  packetslice1[1024*1024]     = {0};
static int      decode_flag                 = 0;
static AVPacket slice_pkt1                  = {0};
static AVPacket slice_pkt2                  = {0};
//static int      slice_flag                  = 0;
...
- (CompassedFrame *)readFrame {
    ...
    if (bHasPkt && readStramType != UnknownStream)
    {
        ...
        if (stream->codec->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            ...
            memcpy(&(packetslice1[slice_pkt1.size]), pkt.data, pkt.size);
            slice_pkt1.size += pkt.size;
            slice_pkt1.pts = pkt.pts;
            slice_pkt1.data = packetslice1;
            ...
            if (pkt.mark_flag == 2)
            {
                slice_pkt2.size = 0;
                slice_pkt2.pts = 0;
                new_buffer= av_malloc(17*(slice_pkt1.size)/16 + 32);
                memcpy(new_buffer, packetslice1,slice_pkt1.size);
                slice_pkt2.size = slice_pkt1.size;
                slice_pkt2.pts = slice_pkt1.pts;
                slice_pkt2.data = new_buffer;
                decode_flag = 1;
            }
            if (decode_flag == 1) {
                pkt.data = slice_pkt2.data;
                pkt.pts = slice_pkt2.pts;
                pkt.size = slice_pkt2.size;
                ...
                slice_pkt1.size = 0;
                slice_pkt1.pts = 0;
                memset(packetslice1, 0, sizeof(packetslice1));
                slice_pkt1.data = packetslice1;
                decode_flag = 0;
                av_free(new_buffer);
            }
...
FFMpegLowDelayDemuxer *createFFMpegLowDelayDemuxer() {
    FFMpegLowDelayDemuxer *demuxer = [[FFMpegLowDelayDemuxerImp alloc] init];
    return demuxer;
}
